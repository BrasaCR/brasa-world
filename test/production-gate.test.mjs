...
test('checked-in infrastructure gate has a production environment but remains closed on evidence', async () => {
  const result = evaluateProductionGate(await readProductionGate(), await readProductionConfig()); assert.equal(result.ready, false); assert.equal(result.missing.includes('production Wrangler environment'),false); assert.ok(result.missing.includes('custom-domain approval')); assert.ok(result.missing.includes('retention-policy approval'));assert.equal(result.missing.includes('first consumer name'),false);
});
