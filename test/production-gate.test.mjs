...
test('checked-in infrastructure gate is ready while pilot activation remains separate', async () => {
  const result = evaluateProductionGate(await readProductionGate(), await readProductionConfig()); assert.deepEqual(result, { ready: true, target: 'api.brasa.world', missing: [] });
});
