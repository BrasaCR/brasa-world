...
test('hashes API keys, enforces scopes, and reports quota headers', async () => {
  ...
  const governmentExperienceDenied = await worker.fetch(new Request('https://api.brasa.world/v1/government/experiences/service-navigator', { headers: { authorization: `Bearer ${key}` } }), { ASSETS: assets, API_DB: database() }); assert.equal(governmentExperienceDenied.status, 403);
