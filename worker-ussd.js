    ...
    if (level === 1) {
      ...
      try {
        const r = await fetch('https://api.anthropic.com/v1/messages', {
          ...
          body: JSON.stringify({ model: 'claude-haiku-4-5-20251001', max_tokens: 120, system: 'USSD: 2 sentences max. Plain text only. Respond in the exact language the person used.', messages: [{ role: 'user', content: inputs[1] || '' }] }),
        });
        const d = await r.json();
        response = 'END ' + (d?.content?.[0]?.text || 'Visit brasa.world').slice(0, 160);
      } catch {
        response = 'END Visit brasa.world for your rights guide.';
      }
    }
  } else {
    const items = {
