class BrasaLessons extends HTMLElement {
  ...
  async load() {
    ...
    try {
      ...
      url.searchParams.set('schoolId', schoolId); url.searchParams.set('locale', locale);
      ...
      list.replaceChildren(...lessons.map((lesson) => {
      ...
      status.textContent = lessons.length ? `${lessons.length} lesson${lessons.length === 1 ? '' : 's'} available.` : 'No published lessons yet.';
    } catch {
      status.replaceChildren(document.createTextNode('Lessons are temporarily unavailable. '));
      const retry = document.createElement('button'); retry.type = 'button'; retry.textContent = 'Try again'; retry.addEventListener('click', () => this.load(), { once: true }); status.append(retry);
    }
  }
}
